...
namespace VHierarchy
{
    public class VHierarchySceneSelectorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            void rows()
            {
                ...
                void divider()
                {
                    ...
                    var splitterRect = bookmarksRect.SetHeightFromBottom(0).SetHeight(dividerHeight).SetHeightFromMid(1)
                        .AddWidthFromMid(-10);
                ...
                scrollPos = GUI.BeginScrollView(windowRect.AddHeightFromBottom(-firstRowOffsetTop),
                    Vector2.up * scrollPos, windowRect.SetHeight(scrollAreaHeight), GUIStyle.none, GUIStyle.none).y;
        ...
        private void RowGUI(Rect rowRect, SceneEntry entry)
        {
            ...
            var showAdditiveIndicator = curEvent.holdingAlt &&
                                        ((keyboardFocusedEntry?.scenePath).IsNullOrEmpty() ? isHovered : isFocused);
            ...
            void name()
            {
                ...
                var nameText = searchString != ""
                    ? namesFormattedForFuzzySearch_byEntry[entry]
                    : entry.sceneName;
            ...
            void curtain()
            {
                ...
                var curtainXMax = rowRect.xMax -
                                  (showStarButton ? 21 : showEnterHint ? 33 : showAdditiveIndicator ? 80 : 0);


                rowRect.SetHeightFromMid(18).SetXMax(curtainXMax).SetWidthFromRight(curtainWidth)
                    .DrawCurtainLeft(curtainColor);
        ...
        private void OpenScene(SceneEntry entry, bool openAdditive)
        {
            ...
            if (openAdditive)
            {
                openScene(entry.scenePath, true);
            }

            else if (EditorSceneManager.sceneCount == 1)
            {
                openScene(entry.scenePath, false);
            }
            ...
            else
            {
                ...
                EditorSceneManager.CloseScene(sceneToReplace, true);
            }
        }
            ...
            void update()
            {
                ...
                insertDraggedBookmarkAtIndex =
                    GetBookmarkIndex(curEvent.mousePosition.y + draggedBookmarkHoldOffsetY + rowHeight / 2)
                        .Clamp(0, bookmarkedEntries.Count);
            }
        ...
        private void BookmarksAnimations()
        {
            ...
            void droppedBookmark_()
            {
                ...
                droppedBookmarkYTarget = GetBookmarY(bookmarkedEntries.IndexOf(droppedBookmark), false);

                MathUtil.SmoothDamp(ref droppedBookmarkY, droppedBookmarkYTarget, lerpSpeed,
                    ref droppedBookmarkYDerivative, editorDeltaTime);
        ...
        {
            bookmarkedEntries = data.bookmarkedScenePaths
                .Select(r => allEntries.FirstOrDefault(rr => rr.scenePath == r))
                .Where(r => r != null)
                .ToList();
        }
        ...
        private void UpdateSearch()
        {
            bool tryMatch(string name, string query, int[] matchIndexes, ref float cost)
            {
                ...
                for (var i = 1; i < name.Length; i++)
                {
                    ...
                    var curChar = name[i];
                    var nextChar = i + 1 < name.Length ? name[i + 1] : default;

                    var isSeparatedWordStart = separators.Contains(prevChar) && !separators.Contains(curChar);
                    var isCamelcaseHump = (curChar.IsUpper() && prevChar.IsLower()) ||
                ...
                while (iName < name.Length && iQuery < query.Length)
                {
                    ...
                    var nextWordInitialIndex =
                        nextWordInitialsIndexMap[iName]; // wordInitialsIndexes.FirstOrDefault(i => i > iName);
                    var nextWordInitialSymbol =
                        nextWordInitialIndex == default ? default : name[nextWordInitialIndex].ToLower();
            ...
            bool tryMatch_unitySearch(string name, string query, int[] matchIndexes, ref float cost)
            {
                ...
                var matched = FuzzySearch.FuzzyMatch(searchString, name, ref score, matchIndexesList);
            ...
            foreach (var entry in allEntries)
                if (tryMatch(entry.sceneName, searchString, matchIndexes, ref matchCost) ||
                    tryMatch_unitySearch(entry.sceneName, searchString, matchIndexes, ref matchCost))
                {
        ...
        public static void Open(Vector2 rowPos, Scene sceneToReplace)
        {
            if (!data)
            {
                data = CreateInstance<VHierarchyData>();

                AssetDatabase.CreateAsset(data,
                    GetScriptPath("VHierarchy").GetParentPath().CombinePath("vHierarchy Data.asset"));
            }


            instance = CreateInstance<VHierarchySceneSelectorWindow>();
        ...
        public class SceneEntry
        {
            ...
            public string sceneName => scenePath.GetFilename();
        }
    }
}
#endif